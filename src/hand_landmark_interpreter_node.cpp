...
void HandLandmarkInterpreter::landmark_callback(const geometry_msgs::msg::PoseArray::SharedPtr msg)
{
  if (msg->poses.size() >= static_cast<size_t>(HAND_LANDMARK_COUNT)) {
    RCLCPP_INFO_THROTTLE(
      this->get_logger(), *this->get_clock(), 1000, "Received %zu hand landmarks",
      msg->poses.size());
...
void HandLandmarkInterpreter::set_all_fingers_except(
  ...
  const std::vector<geometry_msgs::msg::Pose> & landmarks, const std::string & finger)
{
  ...
  std::vector<double> tip_to_mid = {
    landmarks[config.tip_idx].position.x - landmarks[config.mid_idx].position.x,
    landmarks[config.tip_idx].position.y - landmarks[config.mid_idx].position.y};

  std::vector<double> base_to_mid = {
    landmarks[config.base_idx].position.x - landmarks[config.mid_idx].position.x,
  ...
  double mag_base_to_mid =
    std::sqrt(base_to_mid[0] * base_to_mid[0] + base_to_mid[1] * base_to_mid[1]);
  ...
  double cosine_theta =
    std::min(1.0, std::max(-1.0, dot_product / (mag_tip_to_mid * mag_base_to_mid)));
  double angle_degrees = std::acos(cosine_theta) * 180.0 / M_PI;

  // Clamp angle to observed range
  double clamped_degrees =
    std::max(config.observed_min, std::min(config.observed_max, angle_degrees));

  // Convert angle to percentage - original linear mapping
  double raw_percentage =
    (clamped_degrees - config.observed_min) / (config.observed_max - config.observed_min);
  ...
  if (prev_it != prev_finger_curls_.end()) {
    ...
    if ((finger == "thumb_yaw") || (finger == "thumb_pitch")) {
      smooth_factor =
        std::min(0.9, curl_smooth_factor_ + 0.1);  // Increase smoothing factor for thumb yaw
    }
    transformed_percentage =
      smooth_factor * prev_it->second + (1.0 - smooth_factor) * transformed_percentage;
  }
...
void HandLandmarkInterpreter::process_landmarks(
