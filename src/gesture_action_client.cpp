...
public:
  ...
  explicit GestureActionClient(const std::string & node_name = "gesture_client") : Node(node_name)
  {
    ...
    this->timer_ = this->create_wall_timer(
      std::chrono::seconds(1), std::bind(&GestureActionClient::send_goal, this));
  }
  ...
  void send_goal()
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Sending goal: %s (duration=%.2f, percentage=%.2f)",
      goal_msg.gesture_name.c_str(), goal_msg.duration, goal_msg.percentage);

    auto send_goal_options = rclcpp_action::Client<ExecuteGesture>::SendGoalOptions();
    send_goal_options.feedback_callback = std::bind(
      &GestureActionClient::feedback_callback, this, std::placeholders::_1, std::placeholders::_2);
    send_goal_options.result_callback =
      std::bind(&GestureActionClient::result_callback, this, std::placeholders::_1);
