...
AgilexPiperArmNode::AgilexPiperArmNode(const rclcpp::NodeOptions & options)
: Node("piper_controller_node", options)
{
  ...
  this->declare_parameter<int>("motion_mode", MOTION_MODE_JOINT);
  ...
  // Validate parameters
  if (update_frequency_ <= 0.0 || update_frequency_ > 100.0) {
    RCLCPP_WARN(
      this->get_logger(), "Invalid update frequency %f Hz. Using default 50 Hz.",
      update_frequency_);
  ...
  home_positions_.reserve(NUM_JOINTS);
  ...
  gripper_joint_state_pub_ =
    this->create_publisher<sensor_msgs::msg::JointState>("piper/gripper_joint_states", 10);
  ...
  joint_cmd_sub_ = this->create_subscription<trajectory_msgs::msg::JointTrajectory>(
    "piper/joint_command", 10, [this](const trajectory_msgs::msg::JointTrajectory::SharedPtr msg) {
  ...
  gripper_cmd_sub_ = this->create_subscription<control_msgs::msg::GripperCommand>(
    "piper/gripper_command", 10, [this](const control_msgs::msg::GripperCommand::SharedPtr msg) {
      gripper_command_callback(msg);
    });

  // Create services
  enable_arm_srv_ = this->create_service<std_srvs::srv::SetBool>(
    "piper/enable_arm", [this](
                          const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
                          std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
      enable_arm_service_callback(request, response);
    });
  ...
  home_srv_ = this->create_service<std_srvs::srv::Trigger>(
    "piper/home", [this](
      ...
      home_service_callback(request, response);
    });

  emergency_stop_srv_ = this->create_service<std_srvs::srv::Trigger>(
    "piper/emergency_stop", [this](
                              const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
                              std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
      emergency_stop_service_callback(request, response);
    });

  set_mode_srv_ = this->create_service<std_srvs::srv::SetBool>(
    "piper/set_motion_mode", [this](
                               const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
                               std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
      set_mode_service_callback(request, response);
    });
  ...
  const std::string config_file_path =
    resolve_config_file_path(get_parameter("config_file").as_string());
  if (!load_joint_config(config_file_path)) {
    RCLCPP_ERROR(
      this->get_logger(), "Failed to load joint configuration from %s", config_file_path.c_str());
  } else {
    RCLCPP_INFO(
      this->get_logger(), "Successfully loaded joint configuration from %s",
  ...
  if (!setup_controller_connection()) {
    RCLCPP_WARN(
      this->get_logger(), "Controller not connected. Will attempt connection during operation.");
  }

  // Register parameter callback
  param_callback_handle_ =
    this->add_on_set_parameters_callback([this](const std::vector<rclcpp::Parameter> & params) {
      return on_set_parameters_callback(params);
    });

  // Create update timer
  update_timer_ = this->create_wall_timer(
    std::chrono::duration<double>(1.0 / update_frequency_), [this]() { update_callback(); });
  ...
  if (listen_only_) {
    RCLCPP_INFO(
      this->get_logger(),
      "Running in listen-only mode. Commands will be received but not executed.");
  }
}
...
bool AgilexPiperArmNode::is_controller_ready()
{
...
bool AgilexPiperArmNode::load_joint_config(const std::string & config_file)
{
  try {
    if (!std::filesystem::exists(config_file)) {
      RCLCPP_ERROR(
        this->get_logger(), "Error loading configuration: file not found: %s", config_file.c_str());
      return false;
    }
    ...
    if (!config["arm_config"] || !config["arm_config"]["joints"]) {
      RCLCPP_ERROR(
        this->get_logger(), "Invalid configuration file format in %s", config_file.c_str());
      return false;
    }
    ...
    for (size_t i = 0; i < joints.size() && i < NUM_JOINTS; i++) {
      JointConfig joint;
      ...
      // Use default_position as home position
      if (i < home_positions_.size()) {
        home_positions_[i] = joint.default_position;
      }

      RCLCPP_DEBUG(
        this->get_logger(), "Loaded joint %s: min=%f, max=%f, default/home=%f", joint.name.c_str(),
    ...
    if (config["arm_config"]["update_rate"]) {
      ...
      if (update_rate > 0.0 && update_rate <= 100.0) {
        update_frequency_ = update_rate;
        RCLCPP_INFO(
          this->get_logger(), "Setting update frequency to %f Hz from config", update_frequency_);
      }
    }
    ...
    RCLCPP_ERROR(
      this->get_logger(), "Error loading configuration: %s from file: %s", e.what(),
      config_file.c_str());
    return false;
  }
}
...
void AgilexPiperArmNode::apply_joint_limits_to_sdk()
{
  ...
  for (const auto & [joint_name, config] : joint_config_) {
    RCLCPP_INFO(
      this->get_logger(), "Setting SDK joint limit for %s: min=%f, max=%f", joint_name.c_str(),
...
rcl_interfaces::msg::SetParametersResult AgilexPiperArmNode::on_set_parameters_callback(
    ...
    if (param.get_name() == "arm_enabled") {
      ...
      if (new_state != arm_enabled_) {
        ...
        if (!listen_only_ && is_controller_ready()) {
          ...
          RCLCPP_INFO(
            this->get_logger(), "In listen-only mode: arm enable/disable command not executed");
        }
      }
    } else if (param.get_name() == "motion_mode") {
      const int new_mode = param.as_int();
      if (
        new_mode != motion_mode_ &&
        (new_mode == MOTION_MODE_CARTESIAN || new_mode == MOTION_MODE_JOINT)) {
        motion_mode_ = new_mode;
        RCLCPP_INFO(
          this->get_logger(), "Motion mode set to %s",
          motion_mode_ == MOTION_MODE_CARTESIAN ? "Cartesian" : "Joint");
      ...
      } else if (new_mode != MOTION_MODE_CARTESIAN && new_mode != MOTION_MODE_JOINT) {
      ...
      if (new_state != listen_only_) {
        ...
        RCLCPP_INFO(
          this->get_logger(), "Listen-only mode %s", listen_only_ ? "enabled" : "disabled");
        ...
        if (was_listen_only && !listen_only_ && is_controller_ready()) {
          RCLCPP_INFO(
            this->get_logger(), "Transitioning from listen-only mode, applying pending commands");
          if (arm_enabled_) {
            enable_arm_and_gripper();
          } else {
            disable_arm_and_gripper();
...
void AgilexPiperArmNode::apply_motion_mode()
{
  ...
  RCLCPP_INFO(
    this->get_logger(), "Set to %s motion mode",
    motion_mode_ == MOTION_MODE_CARTESIAN ? "Cartesian" : "joint");
}

void AgilexPiperArmNode::update_callback()
{
  bool is_connected = controller_ && controller_->is_connected();

  if (!is_connected) {
    static bool connection_attempted = false;
    if (!connection_attempted) {
      RCLCPP_WARN(this->get_logger(), "Controller not connected, attempting to connect");

      bool connect_success = controller_->connect_port();
      ...
      if (connect_success) {
        ...
        RCLCPP_WARN(
          this->get_logger(), "Failed to connect to controller. Will not retry automatically.");
      }
      connection_attempted = true;
    } else {
      RCLCPP_WARN_THROTTLE(
        this->get_logger(), *this->get_clock(), 5000, "Controller not connected");
    }
    return;
  }
...
bool AgilexPiperArmNode::validate_joint_command(
  const trajectory_msgs::msg::JointTrajectory::SharedPtr msg) const
{
  ...
  if (msg->joint_names.size() < NUM_JOINTS) {
    RCLCPP_ERROR(
      this->get_logger(), "Joint trajectory command has fewer than %zu joint names (got %zu)",
  ...
  if (msg->points[0].positions.size() < NUM_JOINTS) {
    RCLCPP_ERROR(
      this->get_logger(), "Joint trajectory command has fewer than %zu position values (got %zu)",
      NUM_JOINTS, msg->points[0].positions.size());
    return false;
  }

  // Validate joint names exist in our configuration
  for (const auto & name : msg->joint_names) {
    bool found = std::find(joint_names_.begin(), joint_names_.end(), name) != joint_names_.end();
    if (!found && joint_config_.find(name) == joint_config_.end()) {
      RCLCPP_WARN(this->get_logger(), "Unknown joint name in command: %s", name.c_str());
    }
  }
...
std::optional<std::vector<double>> AgilexPiperArmNode::extract_joint_positions(
  const trajectory_msgs::msg::JointTrajectory::SharedPtr msg) const
{
  ...
  for (size_t i = 0; i < NUM_JOINTS; ++i) {
    ...
    if (it != msg->joint_names.end()) {
      ...
      positions[i] = point.positions[idx];
    } else {
      RCLCPP_WARN(
        this->get_logger(), "Joint %s not found in command, using current position",
        joint_names_[i].c_str());
      positions[i] = joint_positions_[i];
    }
  }
...
void AgilexPiperArmNode::joint_command_callback(
  const trajectory_msgs::msg::JointTrajectory::SharedPtr msg)
{
  ...
  if (!arm_enabled_) {
    RCLCPP_WARN(this->get_logger(), "Arm is not enabled. Cannot send joint command.");
    return;
  }

  auto positions_opt = extract_joint_positions(msg);
  if (!positions_opt) {
    return;
  }
  ...
  for (size_t i = 0; i < NUM_JOINTS; ++i) {
    joint_commands[i] = static_cast<int>(positions[i] * 180.0 / M_PI * 1000.0);
  }

  // Send command to controller
  try {
    if (!controller_->set_joint_angles(
          joint_commands[0], joint_commands[1], joint_commands[2], joint_commands[3],
          joint_commands[4], joint_commands[5])) {
      RCLCPP_ERROR(this->get_logger(), "Failed to send joint command to controller");
    } else {
      RCLCPP_DEBUG(
        this->get_logger(), "Sent joint command to controller: [%d, %d, %d, %d, %d, %d]",
        joint_commands[0], joint_commands[1], joint_commands[2], joint_commands[3],
        joint_commands[4], joint_commands[5]);
    }
  } catch (const std::exception & e) {
    RCLCPP_ERROR(this->get_logger(), "Exception sending joint command: %s", e.what());
  }
}
...
void AgilexPiperArmNode::gripper_command_callback(
  const control_msgs::msg::GripperCommand::SharedPtr msg)
{
  ...
  const uint16_t grippers_effort =
    static_cast<uint16_t>(std::clamp(msg->max_effort * 1000.0, 0.0, 5000.0));
  ...
  if (!controller_->control_gripper(grippers_angle, final_effort, GRIPPER_ENABLE, 0x00)) {
    ...
    RCLCPP_DEBUG(
      this->get_logger(), "Sent gripper command: position=%d (0.001mm), effort=%d (0.001N·m)",
...
void AgilexPiperArmNode::publish_arm_status()
{
  ...
  if ((status.ctrl_mode == 0x02) && arm_enabled_) {
    ...
    RCLCPP_WARN(
      this->get_logger(), "Arm disabled due to teaching mode triggered: %d", status.ctrl_mode);
    RCLCPP_WARN(
      this->get_logger(), "Set parameter arm_enabled back to true to recover from teaching mode!");
  }

  // Pack status fields into UInt8MultiArray
  auto status_msg = std::make_unique<std_msgs::msg::UInt8MultiArray>();
  status_msg->data = {
    static_cast<uint8_t>(status.ctrl_mode),     static_cast<uint8_t>(status.arm_status),
    ...
    static_cast<uint8_t>(arm_enabled_ ? 1 : 0), static_cast<uint8_t>(motion_mode_)};
...
void AgilexPiperArmNode::enable_arm_service_callback(
  const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
  std::shared_ptr<std_srvs::srv::SetBool::Response> response)
{
  if (listen_only_) {
    response->success = false;
    response->message = "Cannot enable/disable arm in listen-only mode";
    return;
  }

  arm_enabled_ = request->data;
  ...
  if (is_controller_ready()) {
    ...
    response->message = arm_enabled_ ? "Arm enabled" : "Arm disabled";
  } else {
    response->success = false;
...
void AgilexPiperArmNode::home_service_callback(
  const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  ...
  if (listen_only_) {
    response->success = false;
    response->message = "Cannot move to home in listen-only mode";
    return;
  }
  ...
  response->success = move_to_home_position();
  response->message =
    response->success ? "Moving to home position" : "Failed to move to home position";
}

void AgilexPiperArmNode::emergency_stop_service_callback(
  const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  (void)request;  // Unused parameter
  ...
  try {
    ...
    response->success = false;
    response->message = std::string("Emergency stop failed: ") + e.what();
  }
}

void AgilexPiperArmNode::set_mode_service_callback(
  const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
  std::shared_ptr<std_srvs::srv::SetBool::Response> response)
{
  if (listen_only_) {
    response->success = false;
    response->message = "Cannot set motion mode in listen-only mode";
    return;
  }
  ...
  // Set motion mode based on request->data: true = Joint mode (1), false = Cartesian mode (0)
  const int new_mode = request->data ? MOTION_MODE_JOINT : MOTION_MODE_CARTESIAN;
  motion_mode_ = new_mode;

  try {
    apply_motion_mode();
    response->success = true;
    response->message = std::string("Motion mode set to ") +
                        (motion_mode_ == MOTION_MODE_CARTESIAN ? "Cartesian" : "Joint") + " mode";

    // Update the parameter to reflect the change
    this->set_parameter(rclcpp::Parameter("motion_mode", motion_mode_));
    ...
    response->success = false;
    response->message = std::string("Failed to set motion mode: ") + e.what();
    RCLCPP_ERROR(this->get_logger(), "Failed to set motion mode: %s", e.what());
  }
}

bool AgilexPiperArmNode::move_to_home_position()
{
  try {
    // Ensure we're in joint motion mode before moving to home position
    if (motion_mode_ != MOTION_MODE_JOINT) {
      RCLCPP_INFO(this->get_logger(), "Switching to joint motion mode for home position movement");
      motion_mode_ = MOTION_MODE_JOINT;
      apply_motion_mode();
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Convert home positions to controller format
    std::array<int, NUM_JOINTS> home_commands;
    for (size_t i = 0; i < NUM_JOINTS; ++i) {
      home_commands[i] = static_cast<int>(home_positions_[i] * 180.0 / M_PI * 1000.0);
    }

    bool success = controller_->set_joint_angles(
      home_commands[0], home_commands[1], home_commands[2], home_commands[3], home_commands[4],
      home_commands[5]);

    return success;
  } catch (const std::exception & e) {
    RCLCPP_ERROR(this->get_logger(), "Failed to move to home position: %s", e.what());
    return false;
  }
}

void AgilexPiperArmNode::perform_emergency_stop()
{
